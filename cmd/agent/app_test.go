...
var agentRuntimePrefixSequence atomic.Uint64
...
func uniqueAgentRuntimePrefix(scope string) string {
	return fmt.Sprintf("memoh:test:runtime-lifecycle:%s:%d:%d:", scope, time.Now().UnixNano(), agentRuntimePrefixSequence.Add(1))
}
...
func TestSessionRuntimeLifecycleClosesBackendWhenStartupFails(t *testing.T) {
	...
	hook := sessionRuntimeLifecycleHook(manager)
...
func TestSessionRuntimeDistributedLifecycleOutlivesFXStartupContext(t *testing.T) {
	...
	prefix := uniqueAgentRuntimePrefix(strings.ReplaceAll(t.Name(), "/", ":"))
