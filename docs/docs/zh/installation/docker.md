...
| **web** | *（核心）* | 网页端（Vue 3） |
| **postgres** | *（核心）* | PostgreSQL，内置 pgvector 记忆向量索引 |
...
- 查询端用 IDF 加权词表，检索快。
- 将稀疏向量写入当前数据库后端，可以在**不另接外部 embedding API** 的情况下做语义级记忆搜索。
...
```bash
POSTGRES_PASSWORD=你的库密码 docker compose --profile browser --profile sparse up -d
```
