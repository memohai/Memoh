...
│   ├── gen-bridge-mtls/        #   Utility for generating bridge mTLS material
│   ├── mcp/                    #   Reserved/legacy MCP command directory; runtime MCP code lives under internal/mcp
...
│   ├── synccaps/               #   Sync provider capability metadata into provider templates
│   └── synclitellmproviders/   #   Sync selected LiteLLM provider templates
...
│   ├── acpfeedback/            #   ACP feedback event helpers
...
│   ├── agentpayload/           #   Agent event payload helpers
...
│   ├── capabilities/           #   Model capability discovery and resolution
...
│   ├── channelaccess/          #   Channel manager/member access resolution
...
│   ├── contextlimit/           #   Output/context budget helpers
...
│   ├── hooks/                  #   Bot hook execution and plugin hook integration
...
│   ├── oauthclients/           #   OAuth client registry/configuration helpers
...
│   ├── video/                  #   Video provider management
│   ├── webhooktunnel/          #   Webhook public URL tunnel lifecycle
...
- Tests should protect behavior, regressions, and contracts that callers rely on. Prefer a small number of high-signal cases over broad tables that repeat the same branch.
- Keep tests that cover security, permissions, secret redaction, lifecycle ordering, persistence, migrations, concurrency, backwards compatibility, public API/HTTP boundaries, protocol wire formats, and bugs that have actually regressed.
- Avoid tests that only restate implementation shape: private helper wrappers, constant string maps, default value mirrors, field-by-field struct copying, function calls that are already covered through a public path, or "does not panic" checks without a meaningful boundary.
- Avoid pinning static catalogs and generated-looking text unless the exact catalog or text is the external contract. For provider templates, profile lists, default directory lists, YAML/TOML output, shell argv, or rendered markdown, prefer semantic assertions against parsed data and a few stable invariants.
- Do not duplicate lower-level contracts at every layer. If a formatter/helper package already exhaustively covers list/choice/range behavior, an integration test should verify that the calling layer wires the helper in, not repeat every formatter branch.
- When many table rows hit the same branch, keep the boundary cases and one representative ordinary case. Add more rows only when they cover a distinct branch, platform quirk, encoding boundary, or named regression.
- When removing or simplifying tests, make sure the remaining suite would still fail for the real regression being guarded. If that cannot be shown, keep the test or replace it with a more behavior-focused assertion.
