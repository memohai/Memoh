<template>
  <div
    ...
    :style="{ '--tab-fade': isActive ? 'var(--card)' : 'var(--ui-hover)' }"
    @auxclick.middle.prevent="close"
  >
    <span class="min-w-0 flex-1 truncate text-xs leading-none">{{ title }}</span>
    <!-- Close is a floating overlay pinned to the right, revealed only on hover,
         ...
         button, keyed to the tab's own background (card when active, ui-hover
         when an inactive tab is hovered) so a long title never collides with the
         X. pointer-events stay off the fade so clicking it still selects the
         tab; only the button itself is interactive. -->
    <span
