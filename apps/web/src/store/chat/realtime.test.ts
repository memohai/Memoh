...
describe('chat realtime controller', () => {
  ...
  it('forwards an abort before a runtime generation is available', () => {
    const { controller, sockets } = makeController()
    controller.startWebSocket('bot-1')

    expect(controller.abortWebSocketStream('stream-early', 'bot-1', 'session-1')).toBe(true)
    expect(sockets[0]!.socket.abort).toHaveBeenCalledWith('stream-early', 'session-1', undefined)
  })
