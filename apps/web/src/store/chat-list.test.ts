...
describe('chat-list store', () => {
  ...
  it('blocks branch mutations while the current session is streaming', async () => {
    api.fetchSessions.mockResolvedValueOnce([
      { id: 'session-1', bot_id: 'bot-1', title: 'Main', type: 'chat' },
    ])
    sendEvents = [
    ...
    expect(store.isSessionStreaming('session-1')).toBe(true)
    await expect(store.forkMessage(ASSISTANT_MESSAGE_ID)).resolves.toEqual({ ok: false })
    await store.switchBranch('branch-fork')

    expect(api.forkSessionBranch).not.toHaveBeenCalled()
    expect(api.setActiveSessionBranch).not.toHaveBeenCalled()
    expect(toast.error).toHaveBeenCalled()

    streamHandler?.({ type: 'end', session_id: 'session-1' } as UIStreamEvent)
    await send
  })

  it('ignores stale branch graph responses after a branch mutation', async () => {
    api.fetchSessions.mockResolvedValueOnce([
      { id: 'session-1', bot_id: 'bot-1', title: 'Main', type: 'chat' },
    ])
    let resolveStaleBranches: (value: Awaited<ReturnType<typeof api.fetchSessionBranches>>) => void = () => {}
    api.fetchSessionBranches
      ...
      .mockImplementationOnce(() => new Promise(resolve => {
        resolveStaleBranches = resolve
      }))
    api.forkSessionBranch.mockResolvedValueOnce({
      active_branch_id: 'branch-fork',
      branches: [
        { id: 'branch-root', session_id: 'session-1' },
        { id: 'branch-fork', session_id: 'session-1', parent_branch_id: 'branch-root', active: true },
    ...
    await store.selectBot('bot-1')
    const staleLoad = store.loadBranches()
    const fork = await store.forkMessage(ASSISTANT_MESSAGE_ID)
    resolveStaleBranches({
      active_branch_id: 'branch-root',
      branches: [{ id: 'branch-root', session_id: 'session-1', active: true }],
    })
    await staleLoad

    expect(fork).toEqual({ ok: true })
    expect(store.branchLoading).toBe(false)
    expect(store.branchGraph?.active_branch_id).toBe('branch-fork')
  })

  it('keeps the latest branch graph when same-session branch loads resolve out of order', async () => {
    api.fetchSessions.mockResolvedValueOnce([
      { id: 'session-1', bot_id: 'bot-1', title: 'Main', type: 'chat' },
    ])
    const store = useChatStore()
    ...
    let resolveFirstLoad: (value: Awaited<ReturnType<typeof api.fetchSessionBranches>>) => void = () => {}
    api.fetchSessionBranches
      .mockImplementationOnce(() => new Promise(resolve => {
    ...
    const firstLoad = store.loadBranches()
    const secondLoad = store.loadBranches()
    await secondLoad
    resolveFirstLoad({
      active_branch_id: 'branch-root',
      branches: [{ id: 'branch-root', session_id: 'session-1', active: true }],
  ...
  it('ignores stale older-message pages after switching sessions', async () => {
    api.fetchSessions.mockResolvedValueOnce([
      { id: 'session-a', bot_id: 'bot-1', title: 'A', type: 'chat' },
      { id: 'session-b', bot_id: 'bot-1', title: 'B', type: 'chat' },
    ])
    api.fetchMessagesUI.mockResolvedValueOnce([
      { id: 'a-current', role: 'user', text: 'current A', timestamp: '2026-01-02T00:00:00Z' },
    ])
    const store = useChatStore()
    ...
    let resolveOlderPage: (value: Awaited<ReturnType<typeof api.fetchMessagesUI>>) => void = () => {}
    api.fetchMessagesUI
      .mockImplementationOnce(() => new Promise(resolve => {
        resolveOlderPage = resolve
      }))
      .mockResolvedValueOnce([
        { id: 'b-current', role: 'user', text: 'current B', timestamp: '2026-01-03T00:00:00Z' },
      ])

    const staleOlderLoad = store.loadOlderMessages()
    await flushPromises()
    await store.selectSession('session-b')
    resolveOlderPage([
      { id: 'a-older', role: 'user', text: 'older A', timestamp: '2026-01-01T00:00:00Z' },
    ])
    await staleOlderLoad

    expect(store.sessionId).toBe('session-b')
    expect(store.messages).toEqual([
      expect.objectContaining({ id: 'b-current', role: 'user', text: 'current B' }),
    ])
  })

  it('does not send while a branch action is in flight', async () => {
    api.fetchSessions.mockResolvedValueOnce([
      { id: 'session-1', bot_id: 'bot-1', title: 'Main', type: 'chat' },
    ])
    const store = useChatStore()
    ...
    let resolveFork: (value: Awaited<ReturnType<typeof api.forkSessionBranch>>) => void = () => {}
    api.forkSessionBranch.mockImplementationOnce(() => new Promise(resolve => {
      resolveFork = resolve
    }))
    api.fetchMessagesUI.mockResolvedValueOnce([])

    const fork = store.forkMessage(ASSISTANT_MESSAGE_ID)
    await flushPromises()
    const send = await store.sendMessage('blocked send')

    expect(send).toEqual({ ok: false, stage: 'startup' })
    expect(sentWSMessages).toHaveLength(0)
  ...
  it('does not send while a session switch is still loading messages', async () => {
    api.fetchSessions.mockResolvedValueOnce([
      { id: 'session-a', bot_id: 'bot-1', title: 'A', type: 'chat' },
      { id: 'session-b', bot_id: 'bot-1', title: 'B', type: 'chat' },
    ])
    ...
    let resolveSessionMessages: (value: Awaited<ReturnType<typeof api.fetchMessagesUI>>) => void = () => {}
    api.fetchMessagesUI.mockImplementationOnce(() => new Promise(resolve => {
      resolveSessionMessages = resolve
    }))

    const selecting = store.selectSession('session-b')
    await flushPromises()
    await flushPromises()

    expect(store.loadingChats).toBe(true)
    expect(store.chatViewBusy).toBe(true)
    await expect(store.sendMessage('blocked while loading')).resolves.toEqual({ ok: false, stage: 'startup' })
    expect(sentWSMessages).toHaveLength(0)
  ...
  it('clears branch switch loading state after navigating away from an in-flight switch', async () => {
    api.fetchSessions.mockResolvedValueOnce([
      { id: 'session-a', bot_id: 'bot-1', title: 'A', type: 'chat' },
      { id: 'session-b', bot_id: 'bot-1', title: 'B', type: 'chat' },
    ])
    api.fetchSessionBranches.mockResolvedValueOnce({
      active_branch_id: 'branch-root',
      branches: [
        { id: 'branch-root', session_id: 'session-a', active: true },
        { id: 'branch-fork', session_id: 'session-a', parent_branch_id: 'branch-root' },
      ],
    })
    api.fetchMessagesUI.mockResolvedValueOnce([])
    const store = useChatStore()
    ...
    const branchSwitch = deferred<Awaited<ReturnType<typeof api.setActiveSessionBranch>>>()
    api.setActiveSessionBranch.mockImplementationOnce(() => branchSwitch.promise)
    const sessionLoad = deferred<Awaited<ReturnType<typeof api.fetchMessagesUI>>>()
    api.fetchMessagesUI.mockImplementationOnce(() => sessionLoad.promise)

    const switching = store.switchBranch('branch-fork')
    await flushPromises()
    expect(store.branchActionLoading).toBe(true)

    const selecting = store.selectSession('session-b')
    await flushPromises()
    branchSwitch.resolve({
      active_branch_id: 'branch-fork',
      branches: [
        { id: 'branch-root', session_id: 'session-a' },
        { id: 'branch-fork', session_id: 'session-a', parent_branch_id: 'branch-root', active: true },
      ],
    })
    await switching

    expect(store.sessionId).toBe('session-b')
    expect(store.branchActionLoading).toBe(false)
    expect(store.loadingChats).toBe(true)
  ...
  it('clears stale branch loading state when a branch mutation supersedes an in-flight branch load', async () => {
    api.fetchSessions.mockResolvedValueOnce([
      { id: 'session-1', bot_id: 'bot-1', title: 'Main', type: 'chat' },
    ])
    const store = useChatStore()
    ...
    const branchLoad = deferred<Awaited<ReturnType<typeof api.fetchSessionBranches>>>()
    api.fetchSessionBranches.mockImplementationOnce(() => branchLoad.promise)
    void store.loadBranches()
    await flushPromises()
    expect(store.branchLoading).toBe(true)

    api.fetchMessagesUI.mockResolvedValueOnce([])
    await expect(store.forkMessage(ASSISTANT_MESSAGE_ID)).resolves.toEqual({ ok: true })

    expect(store.branchLoading).toBe(false)
    expect(store.branchGraph?.active_branch_id).toBe('branch-fork')
    branchLoad.resolve({ branches: [], turns: [] })
  })

  it('does not let a stale refresh overwrite messages after a branch mutation', async () => {
    api.fetchSessions.mockResolvedValueOnce([
      { id: 'session-1', bot_id: 'bot-1', title: 'Main', type: 'chat' },
    ])
    sendEvents = [
      { type: 'start' } as UIStreamEvent,
      { type: 'message', data: { id: 0, type: 'text', content: 'root reply' } } as UIStreamEvent,
      { type: 'end' } as UIStreamEvent,
    ]
    const store = useChatStore()
    ...
    let resolveStaleRefresh: (value: Awaited<ReturnType<typeof api.fetchMessagesUI>>) => void = () => {}
    api.fetchMessagesUI
      .mockImplementationOnce(() => new Promise(resolve => {
        resolveStaleRefresh = resolve
      }))
      .mockResolvedValueOnce([
        { id: 'fork-user', role: 'user', text: 'fork message', timestamp: '2026-01-01T00:01:00Z' },
      ])

    const send = store.sendMessage('root message')
    await flushPromises()
    await flushPromises()

    await expect(store.forkMessage(ASSISTANT_MESSAGE_ID)).resolves.toEqual({ ok: true })
    expect(store.messages).toEqual([
      expect.objectContaining({ id: 'fork-user', role: 'user', text: 'fork message' }),
    ])

    resolveStaleRefresh([
      { id: 'root-user', role: 'user', text: 'stale root message', timestamp: '2026-01-01T00:00:00Z' },
    ])
    await send
  ...
  it('deduplicates older pages against adopted server ids', async () => {
    api.fetchSessions.mockResolvedValueOnce([
      { id: 'session-1', bot_id: 'bot-1', title: 'Main', type: 'chat' },
    ])
    api.fetchMessagesUI.mockResolvedValueOnce([
      { id: 'client-user', role: 'user', text: 'current', timestamp: '2026-01-02T00:00:00Z' },
    ])
    const store = useChatStore()

    await store.selectBot('bot-1')
    ;(store.messages[0] as { serverId?: string }).serverId = 'server-user'

    api.fetchMessagesUI
      .mockResolvedValueOnce([
        { id: 'server-user', role: 'user', text: 'current', timestamp: '2026-01-01T00:00:00Z' },
      ])
      .mockResolvedValueOnce([])

    await expect(store.loadOlderMessages()).resolves.toBe(0)
    expect(store.messages).toHaveLength(1)
    expect(store.messages[0]).toMatchObject({ id: 'client-user', serverId: 'server-user' })
  })
