-- Benchmark scenario: user_input_base_head_request
-- Production query: db/postgres/queries/user_input.sql GetPendingUserInputByBaseHeadRequestID
...
JOIN bot_sessions s ON s.id = uir.session_id
  ...
  AND s.deleted_at IS NULL
WHERE uir.id = $1::uuid
  AND uir.bot_id = $2::uuid
  AND uir.session_id = $3::uuid
