...
func (m *Manager) CleanupBotContainer(ctx context.Context, botID string, preserveData bool) error {
	m.logger.Info("CleanupBotContainer starting",
	...
	if err := m.Delete(ctx, botID, preserveData); err != nil {
		...
		m.logger.Warn("CleanupBotContainer: container not found in containerd",
			slog.String("bot_id", botID))
	}

	m.deleteContainerRecord(ctx, botID)
	m.logger.Info("CleanupBotContainer finished", slog.String("bot_id", botID))
	return nil
}
