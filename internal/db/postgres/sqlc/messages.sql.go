...
func (q *Queries) ListActiveMessagesSinceBySessionBranch(ctx context.Context, arg ListActiveMessagesSinceBySessionBranchParams) ([]ListActiveMessagesSinceBySessionBranchRow, error) {
	rows, err := q.db.Query(ctx, listActiveMessagesSinceBySessionBranch, arg.BranchID, arg.SessionID, arg.CreatedAt)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
WHERE m.session_id = $1
  AND (
    ...
    ORDER BY rb.created_at ASC
    LIMIT 1
  )) WHERE cursor.id = $2::uuid), 2147483647)
        OR (
          bvm.depth = COALESCE((SELECT cursor_bvm.depth FROM bot_history_messages cursor JOIN bot_branch_visible_messages cursor_bvm ON cursor_bvm.message_id = cursor.id AND cursor_bvm.branch_id = COALESCE(s.active_branch_id, (
    SELECT rb.id
    FROM bot_session_branches rb
    ...
    LIMIT 1
          ...
          AND (
            ...
            OR (m.created_at = (SELECT cursor.created_at FROM bot_history_messages cursor WHERE cursor.id = $2::uuid) AND m.id > $2::uuid)
          )
        )
      )
    )
