...
WITH target AS (
  ...
  WHERE turns.session_id = $1
    AND turns.request_message_id = $2
    AND turns.superseded_at IS NULL
  LIMIT 1
),
input_messages AS (
  SELECT ids.message_id, ids.ordinal
  FROM unnest($3::uuid[]) WITH ORDINALITY AS ids(message_id, ordinal)
),
next_seq AS (
  SELECT
    target.id,
...
func (q *Queries) AppendMessagesToHistoryTurnByRequest(ctx context.Context, arg AppendMessagesToHistoryTurnByRequestParams) ([]pgtype.UUID, error) {
	rows, err := q.db.Query(ctx, appendMessagesToHistoryTurnByRequest, arg.SessionID, arg.RequestMessageID, arg.MessageIds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
