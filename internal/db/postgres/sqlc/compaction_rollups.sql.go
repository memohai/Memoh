...
const finalizeCompactionRollup = `-- name: FinalizeCompactionRollup :one
...
request_shape AS MATERIALIZED (
  SELECT
    ...
    COALESCE(BOOL_AND(parent_id <> $2), false) AS excludes_target
...
locked_target AS MATERIALIZED (
  ...
  WHERE compact.id = $2
    AND compact.bot_id = $3
    AND $4::uuid IS NOT NULL
    AND compact.session_id IS NOT DISTINCT FROM $4::uuid
...
locked_parents AS MATERIALIZED (
  SELECT parent.id, parent.bot_id, parent.session_id, parent.status, parent.summary, parent.message_count, parent.error_message, parent.usage, parent.model_id, parent.artifact_version, parent.coverage, parent.anchor_start_ms, parent.anchor_end_ms, parent.artifact_level, parent.parent_ids, parent.superseded_by, parent.superseded_at, parent.completed_at, requested.ordinal
...
parent_stats AS MATERIALIZED (
  SELECT
    ...
    COALESCE(BOOL_AND(
      parent.bot_id = $3
      AND parent.session_id IS NOT DISTINCT FROM $4::uuid
...
direct_source_stats AS MATERIALIZED (
  SELECT
    ...
    COUNT(source.id) FILTER (WHERE
      source.bot_id = $3
      AND source.session_id IS NOT DISTINCT FROM $4::uuid
...
ordered_coverage AS MATERIALIZED (
  SELECT
    coverage.parent_ordinal, coverage.source_ordinal, coverage.source, coverage.created_at_ms,
...
ordered_covered_messages AS MATERIALIZED (
  SELECT
    covered.parent_ordinal, covered.source_ordinal, covered.id, covered.turn_position, covered.turn_message_seq, covered.created_at,
...
gap_stats AS MATERIALIZED (
  ...
  JOIN bot_history_messages candidate
    ON candidate.bot_id = $3
   AND candidate.session_id IS NOT DISTINCT FROM $4::uuid
...
eligible AS MATERIALIZED (
  SELECT target.id, shape.message_count, shape.coverage, shape.anchor_start_ms, shape.anchor_end_ms, shape.artifact_level, shape.parent_ids, shape.topology_revision, shape.range_start_turn_position, shape.range_end_turn_position
...
type FinalizeCompactionRollupParams struct {
	ParentIds      []pgtype.UUID `json:"parent_ids"`
	RollupID       pgtype.UUID   `json:"rollup_id"`
	ScopeBotID     pgtype.UUID   `json:"scope_bot_id"`
	ScopeSessionID pgtype.UUID   `json:"scope_session_id"`
	Summary        string        `json:"summary"`
	Usage          []byte        `json:"usage"`
	ModelID        pgtype.UUID   `json:"model_id"`
}

type FinalizeCompactionRollupRow struct {
	Finalized       bool  `json:"finalized"`
	RequestedCount  int32 `json:"requested_count"`
	MatchedCount    int32 `json:"matched_count"`
	SupersededCount int32 `json:"superseded_count"`
}

func (q *Queries) FinalizeCompactionRollup(ctx context.Context, arg FinalizeCompactionRollupParams) (FinalizeCompactionRollupRow, error) {
	row := q.db.QueryRow(ctx, finalizeCompactionRollup,
		arg.ParentIds,
		arg.RollupID,
		arg.ScopeBotID,
		arg.ScopeSessionID,
	...
	var i FinalizeCompactionRollupRow
	err := row.Scan(
		&i.Finalized,
		&i.RequestedCount,
		&i.MatchedCount,
