...
WHERE b.team_id = app.current_team_id() AND b.id = $6
LIMIT 1
`
...
WHERE bcr.team_id = app.current_team_id() AND bcr.bot_id = $1
`
...
FROM bots b
WHERE b.team_id = app.current_team_id() AND b.id = $1 AND b.owner_user_id = $2
LIMIT 1
`
...
FROM bots b
LEFT JOIN models chat_models ON chat_models.id = b.chat_model_id AND chat_models.team_id = app.current_team_id()
WHERE b.team_id = app.current_team_id() AND b.id = $2
...
WITH updated AS (
  ...
  SET display_name = $1,
      updated_at = now()
  WHERE bots.team_id = app.current_team_id() AND bots.id = $2
