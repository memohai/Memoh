...
DELETE FROM email_oauth_tokens WHERE team_id = app.current_team_id() AND email_provider_id = $1
`
...
SELECT id, email_provider_id, email_address, access_token, refresh_token, expires_at, scope, state, created_at, updated_at, team_id FROM email_oauth_tokens WHERE team_id = app.current_team_id() AND email_provider_id = $1
`
...
SELECT id, email_provider_id, email_address, access_token, refresh_token, expires_at, scope, state, created_at, updated_at, team_id FROM email_oauth_tokens WHERE team_id = app.current_team_id() AND state = $1 AND state != ''
`
