...
func TestRedeemChannelLinkCodeRejectsExpiredRFC3339NanoCode(t *testing.T) {
	ctx := context.Background()
	conn, err := db.OpenSQLite(ctx, config.SQLiteConfig{DSN: ":memory:"})
	if err != nil {
		t.Fatalf("open sqlite: %v", err)
...
`)
	...
	_, err = conn.ExecContext(ctx,
		`INSERT INTO channel_link_codes (token, user_id, expires_at) VALUES (?, ?, ?)`,
		"EXPIRED1", userID, time.Now().UTC().Add(-time.Hour).Format(time.RFC3339Nano),
	)
	if err != nil {
		t.Fatalf("insert link code: %v", err)
	}
	...
	if !errors.Is(err, pgx.ErrNoRows) {
		t.Fatalf("expected no rows for expired code, got %v", err)
	}

	var consumedAt sql.NullString
	if err := conn.QueryRowContext(ctx, `SELECT consumed_at FROM channel_link_codes WHERE token = ?`, "EXPIRED1").Scan(&consumedAt); err != nil {
		t.Fatalf("read consumed_at: %v", err)
	}
	if consumedAt.Valid {
		t.Fatalf("expired link code was consumed: %q", consumedAt.String)
	}
	var bindingCount int
	if err := conn.QueryRowContext(ctx, `SELECT count(*) FROM user_channel_identity_bindings`).Scan(&bindingCount); err != nil {
		t.Fatalf("count bindings: %v", err)
	}
	if bindingCount != 0 {
		t.Fatalf("expired link code created %d bindings", bindingCount)
	}
}
