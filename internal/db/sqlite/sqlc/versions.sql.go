...
func (q *Queries) GetVersionSnapshotRuntimeName(ctx context.Context, arg GetVersionSnapshotRuntimeNameParams) (string, error) {
	row := q.db.QueryRowContext(ctx, getVersionSnapshotRuntimeName, arg.ContainerID, arg.Version)
...
func (q *Queries) InsertVersion(ctx context.Context, arg InsertVersionParams) (ContainerVersion, error) {
	row := q.db.QueryRowContext(ctx, insertVersion, arg.ContainerID, arg.SnapshotID, arg.Version)
...
func (q *Queries) ListVersionsByContainerID(ctx context.Context, containerID string) ([]ListVersionsByContainerIDRow, error) {
	rows, err := q.db.QueryContext(ctx, listVersionsByContainerID, containerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) NextVersion(ctx context.Context, containerID string) (int64, error) {
	row := q.db.QueryRowContext(ctx, nextVersion, containerID)
	var column_1 int64
