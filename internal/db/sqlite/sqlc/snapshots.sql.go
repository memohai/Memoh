...
SELECT
  id, container_id, runtime_snapshot_name, display_name,
  parent_runtime_snapshot_name, snapshotter, source, created_at
FROM snapshots
...
func (q *Queries) GetSnapshotByContainerAndRuntimeName(ctx context.Context, arg GetSnapshotByContainerAndRuntimeNameParams) (Snapshot, error) {
	row := q.db.QueryRowContext(ctx, getSnapshotByContainerAndRuntimeName, arg.ContainerID, arg.RuntimeSnapshotName)
	var i Snapshot
	err := row.Scan(
		&i.ID,
		&i.ContainerID,
...
func (q *Queries) ListSnapshotsByContainerID(ctx context.Context, containerID string) ([]Snapshot, error) {
	rows, err := q.db.QueryContext(ctx, listSnapshotsByContainerID, containerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SELECT
  s.id, s.container_id, s.runtime_snapshot_name, s.display_name,
  s.parent_runtime_snapshot_name, s.snapshotter, s.source, s.created_at,
  cv.version
FROM snapshots s
...
func (q *Queries) ListSnapshotsWithVersionByContainerID(ctx context.Context, containerID string) ([]ListSnapshotsWithVersionByContainerIDRow, error) {
	rows, err := q.db.QueryContext(ctx, listSnapshotsWithVersionByContainerID, containerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
INSERT INTO snapshots (
  id, container_id, runtime_snapshot_name, display_name,
  parent_runtime_snapshot_name, snapshotter, source
)
VALUES (
