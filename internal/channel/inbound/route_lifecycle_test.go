...
func TestRouteLifecycleCleanupCannotAliasStaleHandles(t *testing.T) {
	...
	handoff.Abort()
	if retained := dispatcher.routeLifecycle["route-1"]; retained == nil || len(retained.leases) != 0 {
		t.Fatalf("quiescent route scope = %#v, want retained fence without leases", retained)
	}
