...
func TestHandleReplyWithAgent_InlinesImages(t *testing.T) {
	rc := RenderedContext{
		...
		{
			ReceivedAtMs: 300,
			Content:      []RenderedContentPiece{{Type: "text", Text: `<event type="chat_renamed"/>`}},
		},
	}
...
func TestHandleReplyWithAgent_InlinesImageIntoUserMessageWhoseEditTriggeredRun(t *testing.T) {
	t.Parallel()
	...
	rc := RenderedContext{
		{
			...
			ImageRefs:       []ImageAttachmentRef{{ContentHash: "edited-image", Mime: "image/png"}},
		},
		{
			MessageID:       "newer",
			ReceivedAtMs:    2_000,
			EventCursor:     20,
			LastEventCursor: 20,
			Content:         []RenderedContentPiece{{Type: "text", Text: "newer passive message"}},
		},
	}
	resolver := &fakeRunConfigResolver{
		resolveResult: ResolveRunConfigResult{
		...
		turnResponses: []TurnResponseEntry{{RequestedAtMs: 1_500, Role: "assistant", Content: "previous reply"}},
		inlineFn: func(_ context.Context, _ string, refs []ImageAttachmentRef) []sdk.ImagePart {
			if len(refs) != 1 || refs[0].ContentHash != "edited-image" {
				t.Fatalf("image refs = %#v, want edited-image", refs)
			}
			return []sdk.ImagePart{{Image: "data:image/png;base64,EDITED", MediaType: "image/png"}}
		},
	}
	...
	var editedHasImage, newerHasImage bool
	for _, message := range agent.lastConfig.Messages {
		...
		for _, part := range message.Content {
			...
			editedHasImage = editedHasImage || strings.Contains(text, "edited old message")
			newerHasImage = newerHasImage || strings.Contains(text, "newer passive message")
		}
	}
	if !editedHasImage || newerHasImage {
		t.Fatalf("image placement edited=%t newer=%t, want only the triggering edited message", editedHasImage, newerHasImage)
	}
}
