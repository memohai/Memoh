...
var (
	crossBackendSchemaOnce sync.Once
	crossBackendSchemaErr  error
)
...
func TestPostgresRuntimeFenceAcrossValkeyOwners(t *testing.T) {
	...
	startOwner := func(manager *sessionruntime.Manager, streamID string, fence runtimefence.Fence) {
		...
		_, err := manager.StartRunWithAdmissionBuilderHandle(runCtx, fence.BotID, fence.SessionID, streamID, func(admissionCtx context.Context, handle sessionruntime.RunHandle) (sessionruntime.RunAdmissionView, error) {
...
func openCrossBackendPostgresPool(t *testing.T, ctx context.Context) *pgxpool.Pool {
	...
	if os.Getenv("TEST_POSTGRES_BOOTSTRAP_SCHEMA") == "1" {
		crossBackendSchemaOnce.Do(func() {
			schema, readErr := os.ReadFile("../../db/postgres/migrations/0001_init.up.sql")
			if readErr != nil {
				crossBackendSchemaErr = fmt.Errorf("read canonical postgres schema: %w", readErr)
				return
			}
			if _, execErr := pool.Exec(ctx, string(schema)); execErr != nil {
				crossBackendSchemaErr = fmt.Errorf("apply canonical postgres schema: %w", execErr)
			}
		})
		if crossBackendSchemaErr != nil {
			t.Fatal(crossBackendSchemaErr)
		}
	}
