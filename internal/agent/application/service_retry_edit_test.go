...
func (s *acpReplacementMessageService) PersistRound(
	ctx context.Context,
	...
	s.atomicCalls++
	if s.atomicErr != nil {
		return nil, true, s.atomicErr
	}
	persisted := make([]messagepkg.Message, 0, len(inputs))
	for _, input := range inputs {
		message, err := s.Persist(ctx, input)
		if err != nil {
			return nil, true, err
		}
		persisted = append(persisted, message)
	}
	requestMessageID := strings.TrimSpace(options.Replacement.RequestMessageID)
	if requestMessageID == "" {
	...
	s.replacement = []string{
		sessionID,
		options.Replacement.OldTurnID,
		requestMessageID,
		firstAssistantID(persisted),
...
func TestACPRetryReusesUserAndReplacesLatestTurn(t *testing.T) {
	...
	if messages.atomicCalls != 1 || messages.replaceCalls != 0 {
		t.Fatalf("atomic calls = %d, ReplaceTurn calls = %d, want 1 and 0", messages.atomicCalls, messages.replaceCalls)
	}
...
func TestACPRetryAtomicFailureDoesNotFallbackToReplaceTurn(t *testing.T) {
	t.Parallel()

	atomicErr := errors.New("replace transaction failed")
	messages := &acpReplacementMessageService{
		messages: map[string]messagepkg.Message{
			"user-old":      {ID: "user-old", Role: "user", Content: newTextContent("original")},
			"assistant-old": {ID: "assistant-old", Role: "assistant"},
	...
	if err == nil {
		t.Fatal("RetryLatestMessageWS() error = nil, want atomic persistence failure")
	}
	if got := apperror.CodeOf(err); got != apperror.CodeChatTurnReplacementFailed {
		t.Fatalf("error code = %q, want %q", got, apperror.CodeChatTurnReplacementFailed)
	}
	if got := apperror.CauseOf(err); !errors.Is(got, atomicErr) {
		t.Fatalf("private cause = %v, want atomic persistence error", got)
	}
	if strings.Contains(err.Error(), atomicErr.Error()) {
		t.Fatalf("public error leaked private cause: %q", err)
	}
	if messages.atomicCalls != 1 || messages.replaceCalls != 0 || len(messages.persisted) != 0 {
		t.Fatalf(
			"atomic calls = %d, ReplaceTurn calls = %d, persisted messages = %d; want 1, 0, 0",
			messages.atomicCalls,
			messages.replaceCalls,
			len(messages.persisted),
		)
	}
}
