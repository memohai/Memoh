...
func (p *EmailProvider) Tools(_ context.Context, session SessionContext) ([]sdk.Tool, error) {
	...
	return []sdk.Tool{
		{
			Name: "email.accounts_list", Description: "List the email accounts (provider bindings) configured for this bot, including provider IDs, email addresses, and permissions.",
		...
		{
			Name: "email.send", Description: "Send an email via the bot's configured email provider. Requires write permission.",
		...
		{
			Name: "email.list", Description: "List emails from the mailbox (newest first). Supports pagination. Requires read permission.",
			Parameters: map[string]any{
				"type": "object",
				"properties": map[string]any{
		...
		{
			Name: "email.read", Description: "Read the full content of an email by its UID. Requires read permission.",
			Parameters: map[string]any{
				"type": "object",
				"properties": map[string]any{
