...
func (n Notification) FormatForAgent() string {
	...
	if n.Stalled {
		fmt.Fprintf(&b, "  <suggestion>This command appears to be waiting for interactive input. Kill it with background.status and retry with a non-interactive flag (e.g. -y, --yes, --non-interactive).</suggestion>\n")
	}
	fmt.Fprintf(&b, "</task-notification>")
	return b.String()
}
