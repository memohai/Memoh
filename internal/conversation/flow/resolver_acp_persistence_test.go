...
type firstProjectionFailureMessageService struct {
	recordingMessageService
	persistCalls int
}

func (s *firstProjectionFailureMessageService) Persist(ctx context.Context, input messagepkg.PersistInput) (messagepkg.Message, error) {
	s.persistCalls++
	if s.persistCalls == 1 {
		return messagepkg.Message{}, errors.New("persist pending projection")
	}
	return s.recordingMessageService.Persist(ctx, input)
}

func (s *firstProjectionFailureMessageService) PersistTurnResponseTail(_ context.Context, inputs []messagepkg.PersistInput) ([]messagepkg.Message, error) {
