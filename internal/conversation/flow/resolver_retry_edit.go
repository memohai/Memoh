...
func (r *Resolver) replacePersistedTurn(
	...
	if state := replacementPersistenceFromContext(ctx); state != nil && state.atomicCommitted {
	...
	var forkAnchorUpdate *forkAnchorUpdate
	if state != nil {
		if !state.forkAnchorPrepared {
			var err error
			state.forkAnchor, err = r.prepareForkAnchorUpdate(ctx, req.SessionID, state.replacedTailStartMessageID)
			if err != nil {
				return fmt.Errorf("prepare replacement fork anchor: %w", err)
			}
			state.forkAnchorPrepared = true
		}
		forkAnchorUpdate = state.forkAnchor
	} else {
		var err error
		forkAnchorUpdate, err = r.prepareForkAnchorUpdate(ctx, req.SessionID, req.HistoryCutoffBeforeMessageID)
		if err != nil {
			return fmt.Errorf("prepare replacement fork anchor: %w", err)
		}
	}
	...
	if err := r.applyForkAnchorUpdate(context.WithoutCancel(ctx), req.SessionID, forkAnchorUpdate); err != nil {
		return err
	}
	r.publishReplacementMessageCreated(req.BotID, persisted)
	return nil
...
func (r *Resolver) applyForkAnchorUpdate(ctx context.Context, sessionID string, update *forkAnchorUpdate) error {
	sessionID = strings.TrimSpace(sessionID)
	...
	if _, err := r.sessionService.UpdateMetadata(ctx, sessionID, update.metadata); err != nil {
		return fmt.Errorf("persist fork anchor update: %w", err)
	}
	return nil
}
