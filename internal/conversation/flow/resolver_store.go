...
func (r *Resolver) storeMessages(ctx context.Context, req conversation.ChatRequest, messages []conversation.ModelMessage, modelID string, opts storeRoundOptions) ([]messagepkg.Message, error) {
	...
	if _, fenced := runtimefence.FromContext(ctx); fenced {
		...
		if replacement := replacementPersistenceFromContext(ctx); replacement != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("persist runtime-fenced message round: %w", err)
		}
		if !handled {
			return nil, errors.New("message service declined runtime-fenced round persistence")
		}
		if replacement := replacementPersistenceFromContext(ctx); replacement != nil {
