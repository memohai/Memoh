...
-- RLS enforcement (0107): FORCE ROW LEVEL SECURITY on every tenant table so the
-- team_isolation policy is a hard backstop, and create the non-owner,
-- non-superuser memoh_app runtime role the application connects as. The FORCE
-- list is kept verbatim in sync with the ENABLE list above and in
-- 0105_team_multitenancy.up.sql; tables created by later migrations
-- (media_assets, browser_contexts, tasks) are guarded by to_regclass and get
-- FORCEd by 0107 on fresh installs.
DO $$
DECLARE t TEXT;
BEGIN
