...
LEFT JOIN bot_sessions ps ON ps.id = s.parent_session_id AND ps.team_id = app.current_team_id()
...
LEFT JOIN bot_sessions s ON s.id = m.session_id AND s.team_id = app.current_team_id()
LEFT JOIN bot_sessions ps ON ps.id = s.parent_session_id AND ps.team_id = app.current_team_id()
LEFT JOIN models mo ON mo.id = m.model_id AND mo.team_id = app.current_team_id()
LEFT JOIN providers lp ON lp.id = mo.provider_id AND lp.team_id = app.current_team_id()
WHERE m.team_id = app.current_team_id() AND m.bot_id = sqlc.arg(bot_id)
  AND m.usage IS NOT NULL
  AND m.created_at >= sqlc.arg(from_time)
  AND m.created_at < sqlc.arg(to_time)
