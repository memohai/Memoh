...
SELECT count(*) FROM bot_heartbeat_logs WHERE tenant_id = app.current_tenant_id() AND bot_id = $1;

-- name: DeleteHeartbeatLogsByBot :exec
DELETE FROM bot_heartbeat_logs WHERE tenant_id = app.current_tenant_id() AND bot_id = $1;
