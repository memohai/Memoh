...
WHERE cv.team_id = app.current_team_id() AND s.team_id = app.current_team_id() AND cv.container_id = sqlc.arg(container_id)
ORDER BY cv.version ASC;

-- name: NextVersion :one
SELECT COALESCE(MAX(version), 0) + 1 FROM container_versions WHERE team_id = app.current_team_id() AND container_id = sqlc.arg(container_id);
