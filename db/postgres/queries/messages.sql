...
SELECT CASE
  WHEN sqlc.narg(persist_branch_id)::uuid IS NULL THEN TRUE
  WHEN sqlc.narg(persist_turn_id)::uuid IS NOT NULL THEN EXISTS (
    SELECT 1
    FROM bot_history_turns t
    JOIN branch_path bp ON bp.branch_id = t.branch_id
    LEFT JOIN bot_history_messages fm ON fm.id = t.final_assistant_message_id
    WHERE t.id = sqlc.narg(persist_turn_id)::uuid
      AND t.session_id = sqlc.arg(session_id)
      AND t.branch_id = sqlc.narg(persist_branch_id)::uuid
      AND (
        bp.depth = 0
  ...
  ELSE EXISTS (
    SELECT 1
    FROM branch_path bp
    WHERE bp.branch_id = sqlc.narg(persist_branch_id)::uuid
      AND bp.depth = 0
  )
