...
SELECT * FROM storage_providers WHERE tenant_id = app.current_tenant_id() AND id = sqlc.arg(id);

-- name: GetStorageProviderByName :one
SELECT * FROM storage_providers WHERE tenant_id = app.current_tenant_id() AND name = sqlc.arg(name);

-- name: ListStorageProviders :many
SELECT * FROM storage_providers WHERE tenant_id = app.current_tenant_id() ORDER BY created_at DESC;
...
SELECT * FROM bot_storage_bindings WHERE tenant_id = app.current_tenant_id() AND bot_id = sqlc.arg(bot_id);
...
ON CONFLICT (tenant_id, message_id, content_hash) DO UPDATE SET
  role = EXCLUDED.role,
...
WHERE tenant_id = app.current_tenant_id() AND message_id = sqlc.arg(message_id)
ORDER BY ordinal ASC;

-- name: ListMessageAssetsBatch :many
SELECT id AS rel_id, message_id, role, ordinal, content_hash, name, metadata
FROM bot_history_message_assets
WHERE tenant_id = app.current_tenant_id() AND message_id = ANY(sqlc.arg(message_ids)::uuid[])
...
WHERE a.tenant_id = app.current_tenant_id() AND m.tenant_id = app.current_tenant_id() AND m.bot_id = sqlc.arg(bot_id);

-- name: DeleteMessageAssets :exec
DELETE FROM bot_history_message_assets WHERE tenant_id = app.current_tenant_id() AND message_id = sqlc.arg(message_id);
