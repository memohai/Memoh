...
SELECT * FROM email_oauth_tokens WHERE team_id = app.current_team_id() AND email_provider_id = sqlc.arg(email_provider_id);

-- name: GetEmailOAuthTokenByState :one
SELECT * FROM email_oauth_tokens WHERE team_id = app.current_team_id() AND state = sqlc.arg(state) AND state != '';
...
DELETE FROM email_oauth_tokens WHERE team_id = app.current_team_id() AND email_provider_id = sqlc.arg(email_provider_id);
