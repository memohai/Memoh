...
ON CONFLICT (tenant_id, email_provider_id) DO UPDATE SET
...
SELECT * FROM email_oauth_tokens WHERE tenant_id = app.current_tenant_id() AND email_provider_id = sqlc.arg(email_provider_id);

-- name: GetEmailOAuthTokenByState :one
SELECT * FROM email_oauth_tokens WHERE tenant_id = app.current_tenant_id() AND state = sqlc.arg(state) AND state != '';
...
DELETE FROM email_oauth_tokens WHERE tenant_id = app.current_tenant_id() AND email_provider_id = sqlc.arg(email_provider_id);
