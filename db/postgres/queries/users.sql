...
SET username = sqlc.arg(username),
    ...
    updated_at = now()
WHERE team_id = app.current_team_id() AND id = sqlc.arg(user_id)
RETURNING *;
...
SELECT * FROM users WHERE team_id = app.current_team_id() AND (username = sqlc.arg(identity) OR email = sqlc.arg(identity));

-- name: GetAccountByUserID :one
SELECT * FROM users WHERE team_id = app.current_team_id() AND id = sqlc.arg(user_id);
