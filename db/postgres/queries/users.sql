...
SET username = sqlc.arg(username),
    ...
    updated_at = now()
WHERE tenant_id = app.current_tenant_id() AND id = sqlc.arg(user_id)
RETURNING *;
...
SELECT * FROM users WHERE tenant_id = app.current_tenant_id() AND (username = sqlc.arg(identity) OR email = sqlc.arg(identity));

-- name: GetAccountByUserID :one
SELECT * FROM users WHERE tenant_id = app.current_tenant_id() AND id = sqlc.arg(user_id);
