...
SELECT * FROM bot_email_bindings WHERE tenant_id = app.current_tenant_id() AND id = sqlc.arg(id);

-- name: GetBotEmailBindingByBotAndProvider :one
SELECT * FROM bot_email_bindings
WHERE tenant_id = app.current_tenant_id() AND bot_id = sqlc.arg(bot_id) AND email_provider_id = sqlc.arg(email_provider_id);
...
SELECT * FROM bot_email_bindings
WHERE tenant_id = app.current_tenant_id() AND email_provider_id = sqlc.arg(email_provider_id)
ORDER BY created_at DESC;

-- name: ListReadableBindingsByProvider :many
SELECT * FROM bot_email_bindings
WHERE tenant_id = app.current_tenant_id() AND email_provider_id = sqlc.arg(email_provider_id) AND can_read = TRUE
ORDER BY created_at DESC;
...
DELETE FROM bot_email_bindings WHERE tenant_id = app.current_tenant_id() AND id = sqlc.arg(id);
