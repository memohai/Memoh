...
FROM bots
LEFT JOIN models AS chat_models ON chat_models.id = bots.chat_model_id AND chat_models.team_id = app.current_team_id()
LEFT JOIN models AS heartbeat_models ON heartbeat_models.id = bots.heartbeat_model_id AND heartbeat_models.team_id = app.current_team_id()
LEFT JOIN models AS compaction_models ON compaction_models.id = bots.compaction_model_id AND compaction_models.team_id = app.current_team_id()
LEFT JOIN models AS title_models ON title_models.id = bots.title_model_id AND title_models.team_id = app.current_team_id()
LEFT JOIN models AS image_models ON image_models.id = bots.image_model_id AND image_models.team_id = app.current_team_id()
LEFT JOIN search_providers ON search_providers.id = bots.search_provider_id AND search_providers.team_id = app.current_team_id()
LEFT JOIN fetch_providers ON fetch_providers.id = bots.fetch_provider_id AND fetch_providers.team_id = app.current_team_id()
LEFT JOIN memory_providers ON memory_providers.id = bots.memory_provider_id AND memory_providers.team_id = app.current_team_id()
LEFT JOIN models AS tts_models ON tts_models.id = bots.tts_model_id AND tts_models.team_id = app.current_team_id()
LEFT JOIN models AS transcription_models ON transcription_models.id = bots.transcription_model_id AND transcription_models.team_id = app.current_team_id()
LEFT JOIN models AS video_models ON video_models.id = bots.video_model_id AND video_models.team_id = app.current_team_id()
WHERE bots.team_id = app.current_team_id() AND bots.id = $1;
...
FROM updated
LEFT JOIN models AS chat_models ON chat_models.id = updated.chat_model_id AND chat_models.team_id = app.current_team_id()
LEFT JOIN models AS heartbeat_models ON heartbeat_models.id = updated.heartbeat_model_id AND heartbeat_models.team_id = app.current_team_id()
LEFT JOIN models AS compaction_models ON compaction_models.id = updated.compaction_model_id AND compaction_models.team_id = app.current_team_id()
LEFT JOIN models AS title_models ON title_models.id = updated.title_model_id AND title_models.team_id = app.current_team_id()
LEFT JOIN models AS image_models ON image_models.id = updated.image_model_id AND image_models.team_id = app.current_team_id()
LEFT JOIN search_providers ON search_providers.id = updated.search_provider_id AND search_providers.team_id = app.current_team_id()
LEFT JOIN fetch_providers ON fetch_providers.id = updated.fetch_provider_id AND fetch_providers.team_id = app.current_team_id()
LEFT JOIN memory_providers ON memory_providers.id = updated.memory_provider_id AND memory_providers.team_id = app.current_team_id()
LEFT JOIN models AS tts_models ON tts_models.id = updated.tts_model_id AND tts_models.team_id = app.current_team_id()
LEFT JOIN models AS transcription_models ON transcription_models.id = updated.transcription_model_id AND transcription_models.team_id = app.current_team_id()
LEFT JOIN models AS video_models ON video_models.id = updated.video_model_id AND video_models.team_id = app.current_team_id();
