...
SELECT * FROM provider_oauth_tokens WHERE team_id = app.current_team_id() AND provider_id = sqlc.arg(provider_id);

-- name: GetProviderOAuthTokenByState :one
SELECT * FROM provider_oauth_tokens WHERE team_id = app.current_team_id() AND state = sqlc.arg(state) AND state != '';
...
DELETE FROM provider_oauth_tokens WHERE team_id = app.current_team_id() AND provider_id = sqlc.arg(provider_id);
