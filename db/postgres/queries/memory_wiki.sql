...
SELECT * FROM memory_nodes
WHERE team_id = app.current_team_id() AND bot_id = $1 AND layer = $2
ORDER BY captured_at ASC;

-- name: ListMemoryNodesByBotProfile :many
SELECT * FROM memory_nodes
WHERE team_id = app.current_team_id() AND bot_id = $1 AND profile_ref = $2
ORDER BY captured_at ASC;
...
WHERE team_id = app.current_team_id() AND bot_id = $1 AND src_node = $2
...
SELECT * FROM memory_edges
WHERE team_id = app.current_team_id() AND bot_id = $1 AND rel = $2;

-- name: DeleteMemoryEdgesForNode :exec
DELETE FROM memory_edges
WHERE team_id = app.current_team_id() AND bot_id = $1 AND (src_node = $2 OR dst_node = $2);
