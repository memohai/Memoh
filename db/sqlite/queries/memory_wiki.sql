...
SELECT * FROM memory_nodes
WHERE bot_id = sqlc.arg(bot_id) AND layer = sqlc.arg(layer)
ORDER BY captured_at ASC;

-- name: ListMemoryNodesByBotProfile :many
SELECT * FROM memory_nodes
WHERE bot_id = sqlc.arg(bot_id) AND profile_ref = sqlc.arg(profile_ref)
ORDER BY captured_at ASC;
...
WHERE bot_id = sqlc.arg(bot_id) AND src_node = sqlc.arg(src_node)
...
SELECT * FROM memory_edges
WHERE bot_id = sqlc.arg(bot_id) AND rel = sqlc.arg(rel);

-- name: DeleteMemoryEdgesForNode :exec
DELETE FROM memory_edges
WHERE bot_id = sqlc.arg(bot_id) AND (src_node = sqlc.arg(node_id) OR dst_node = sqlc.arg(node_id));
