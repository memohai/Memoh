...
-- Remove bot-level video model configuration.
...
WHERE type = 'video'
   OR provider_id IN (
    ...
    WHERE client_type IN ('openrouter-video', 'modelark-video', 'volcengine-video')
   );
...
ALTER TABLE bots DROP COLUMN video_model_id;
