...
function sameRuntimeSnapshotIdentity(left: SessionruntimeSnapshot | undefined, right: SessionruntimeSnapshot): boolean {
  if (!left) return false
  const leftRun = left.current_run_view
  const rightRun = right.current_run_view
  return sameRuntimeSnapshotTarget(left, right)
    && Boolean(leftRun) === Boolean(rightRun)
    && trimmedString(leftRun?.stream_id) === trimmedString(rightRun?.stream_id)
    && trimmedString(leftRun?.generation) === trimmedString(rightRun?.generation)
}
...
function resync(state: SessionRuntimeReducerState, reason: Extract<SessionRuntimeReduction, { kind: 'resync' }>['reason']): SessionRuntimeReduction {
  ...
  return {
    kind: 'resync',
    state: { snapshot: state.snapshot, epoch: epoch || undefined },
    reason,
  }
}
...
export function reduceSessionRuntimeSnapshot(
  ...
  if (currentEpoch && !epoch) return resync(state, 'epoch_mismatch')
  const sameEpoch = Boolean(epoch && currentEpoch && epoch === currentEpoch)
  const legacyEpoch = !epoch && !currentEpoch
  if (seq !== undefined && state.seq !== undefined) {
    if (sameEpoch && seq < state.seq) return { kind: 'ignored', state }
    if (sameEpoch && seq === state.seq) {
      if (!authoritative) return { kind: 'ignored', state }
      if (!sameRuntimeSnapshotIdentity(state.snapshot, snapshot)) return resync(state, 'stream_mismatch')
    }
    if (legacyEpoch && !authoritative && seq <= state.seq) return { kind: 'ignored', state }
  }
  ...
  return {
    ...
    state: {
      ...
      epoch: epoch || currentEpoch || undefined,
      seq,
    },
  }
}
...
export function reduceSessionRuntimeDelta(
  ...
  const selfContained = Boolean(delta.current_run_view)
  if (!selfContained && (state.seq === undefined || seq !== state.seq + 1)) {
    return resync(state, 'sequence_gap')
  }

  let snapshot = state.snapshot ? cloneSnapshotForDelta(state.snapshot) : undefined
  if (!snapshot && selfContained) {
    snapshot = {
      bot_id: botId.trim(),
  ...
  if (delta.current_run_view) {
    ...
    return { kind: 'applied', state: { snapshot, epoch: epoch || undefined, seq } }
  }
  ...
  for (const incoming of delta.message_upserts ?? []) {
    const targetMessages: ConversationUiMessage[] = run.messages ?? (run.messages = [])
    const index = targetMessages.findIndex(message => message.id === incoming.id)
